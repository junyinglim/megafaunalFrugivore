...
presNat_facherb_overlap <- readRDS(file.path(projdata.dir, "mammal_presnat_facherb_occ_raw.rds"))
...
presNat_overlap_res <- merge(presNat_overlap, tdwg_data[c("LEVEL_3_CO", "ISISLAND", "CONTINENT", "GeologicalOrigin")])

presNat_facherb_overlap_res <- merge(presNat_facherb_overlap, tdwg_data[c("LEVEL_3_CO", "ISISLAND", "CONTINENT", "GeologicalOrigin")])
...
presNat_facherb_overlap_res$ISISLAND[presNat_facherb_overlap_res$LEVEL_3_CO %in% c("SUM", "JAW", "BOR", "SRL", "NWG")] <- 0 # sumatra, jawa, borneo, sri lanka, and new guinea (SIC = sicilia)
presNat_facherb_overlap_res$ISISLAND[presNat_facherb_overlap_res$GeologicalOrigin == "continental"] <- 0
...
presNat_facherb_overlap_res$CONTINENT_COARSE <- as.vector(presNat_facherb_overlap_res$CONTINENT)
presNat_facherb_overlap_res$CONTINENT_COARSE[presNat_facherb_overlap_res$CONTINENT_COARSE == "AFRICA"] <- "AFRICA"
presNat_facherb_overlap_res$CONTINENT_COARSE[presNat_facherb_overlap_res$CONTINENT_COARSE %in% c("NORTHERN AMERICA", "SOUTHERN AMERICA")] <- "AMERICA"
presNat_facherb_overlap_res$CONTINENT_COARSE[presNat_facherb_overlap_res$CONTINENT_COARSE %in% c("EUROPE", "ASIA-TEMPERATE", "ASIA-TROPICAL")] <- "EURASIA"
presNat_facherb_overlap_res$CONTINENT_COARSE[presNat_facherb_overlap_res$CONTINENT_COARSE %in% c("AUSTRALASIA")] <- "AUSTRALASIA"
...
table(presNat_facherb_overlap_res$CONTINENT_COARSE)
...
facherb_flagged <- ddply(.data = presNat_facherb_overlap_res,
                         .variables = .(SpecName),
                         .fun = function(x){
                           flaglist <- ddply(.data = x,
                                             .variables = .(gridcell),
                           ...
                           return(flaglist)},
                         .progress = "text")
facherb_flagged_subset <- subset(facherb_flagged, island_flag == 1 | diffcont_flag == 1)
write.csv(facherb_flagged_subset, file.path(projdata.dir, "flaggedPresentNaturalFacHerbRanges.csv"))
